...
For this repository that is `andreashasse` and `ecto_spectral`.
...
1. **Write a failing test first.** Add a test to the appropriate `test/` file that reproduces the exact problem described. The test must fail before the fix.
   - `test/ecto_spectral/jsonb_test.exs` for callback behaviour that needs no database.
   - `test/ecto_spectral/postgres_test.exs` for anything that has to touch real `jsonb`. Prefer this one whenever the claim involves what the column actually holds.
   - Support modules live in `test/support/`. A new Spectral type goes in `types.ex`, a new column needs a field in `schemas.ex` and a line in the migration under `priv/repo/migrations/`.
2. Make sure a database is running (`make db`), then run `mix test path/to/test_file.exs` to confirm the test fails.
...
2. Consider: **Is this a pattern that will recur?** If so, add a concise guideline to `CLAUDE.md` under the appropriate section to prevent the same issue from appearing in future code, creating the file if it does not exist yet. Keep CLAUDE.md additions short and actionable — one bullet or sentence.
...
All checks must pass before finishing. `make ci` covers compile, tests, Credo, Dialyzer, formatting, `mix docs` and `mix hex.build`.
