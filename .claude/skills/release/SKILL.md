---
name: release
description: Prepare and publish a new release of EctoSpectral. Updates CHANGELOG.md, mix.exs, and README.md with version information, analyzes git commits to determine semantic version, and guides through the release process.
---

# Release Skill

You are helping to prepare a new release of the EctoSpectral library.
...
This library wraps two moving targets, so watch for two extra sources of breakage:
- A change to the `spectral` version bound in `mix.exs`.
- A change to the `ecto` version bound, or to behaviour that depends on Ecto internals. `cast/2`, `load/3` and `dump/3` all rely on how `Ecto.Type` dispatches to parameterized types.
...
### mix.exs
- Update the `@version` attribute to the new version

### README.md
- Find and update the installation example: `{:ecto_spectral, "~> X.Y.Z"}`
...
The bracketed heading is not cosmetic: `make release` extracts the GitHub release notes by matching `## [X.Y.Z]` in this file.

## Step 6: Run Quality Checks

The test suite needs a real Postgres instance, so start one first:
...
`make ci` covers compile, tests, Credo, Dialyzer, formatting, `mix docs` and `mix hex.build`. All of it must pass before proceeding.
